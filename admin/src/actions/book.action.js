...
export const getBook = () => async (dispatch, getState) => {
  let res;
  try {
    res = await axios.post("http://localhost:8080/book/allshoes", {
...
export const getAuthor = () => async (dispatch, getState) => {
  ...
  try {
    res = await axios.get(
      "http://localhost:8080/author/all/" + getState().bookReducers.author.page
    );
  } catch (err) {
    return;
  }
...
export const addBook =
  (id_category, name, price, release_date, describe, id_nsx, file) =>
  async (dispatch, getState) => {
    ...
    let res;
    try {
      res = await axios.post("http://localhost:8080/admin/addshoes", data);
    } catch (err) {
